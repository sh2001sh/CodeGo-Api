...
func (a *TaskAdaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (taskID string, taskData []byte, taskErr *dto.TaskError) {
	...
	err = platformencoding.Unmarshal(responseBody, &kResp)
	if err != nil {
		taskErr = taskx.TaskErrorWrapper(err, "unmarshal_response_failed", http.StatusInternalServerError)
		return
	}
	if kResp.Code != 0 {
		taskErr = taskx.TaskErrorWrapperLocal(fmt.Errorf("%s", kResp.Message), "task_failed", http.StatusBadRequest)
		return
	}
...
func (a *TaskAdaptor) GetChannelName() string { return "kling" }
...
func (a *TaskAdaptor) ParseTaskResult(respBody []byte) (*relaycommon.TaskInfo, error) {
	...
	err := platformencoding.Unmarshal(respBody, &resPayload)
	if err != nil {
...
func (a *TaskAdaptor) ConvertToOpenAIVideo(originTask *workflowschema.Task) ([]byte, error) {
	...
	if err := platformencoding.Unmarshal(originTask.Data, &klingResp); err != nil {
	...
	if klingResp.Code != 0 && klingResp.Message != "" {
		openAIVideo.Error = &dto.OpenAIVideoError{Message: klingResp.Message, Code: fmt.Sprintf("%d", klingResp.Code)}
	}
	if data := klingResp.Data; data.TaskStatus == "failed" {
		openAIVideo.Error = &dto.OpenAIVideoError{Message: data.TaskStatusMsg}
	}
