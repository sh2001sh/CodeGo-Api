...
// TODO: when a codego-api is enabled, check the pricing here
// 1 === $0.002 / 1K tokens
// 1 === ￥0.014 / 1k tokens
...
func GetModelPrice(name string, printErr bool) (float64, bool) {
	...
	if strings.HasSuffix(name, CompactModelSuffix) {
		...
		if !ok {
			if printErr {
				platformobservability.SysError("model price not found: " + name)
			}
			return -1, false
		}
		return price, true
...
func GetModelRatio(name string) (float64, bool, string) {
	...
	if !ok {
		...
		return 37.5, platformops.IsSelfUseModeEnabled(), name
	}
...
func DefaultModelRatio2JSONString() string {
	...
	if err != nil {
		platformobservability.SysError("error marshalling model ratio: " + err.Error())
	}
...
func FormatMatchingModelName(name string) string {
	if name == "gpt-5.4-2026-03-05" {
		name = "gpt-5.4"
	} else if name == "gpt-5.4-pro-2026-03-05" {
		name = "gpt-5.4-pro"
	}
