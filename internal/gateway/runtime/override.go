...
func APIErrorFromParamOverride(err *ParamOverrideReturnError) *types.APIError {
	if err == nil {
	...
	opts := make([]types.APIErrorOptions, 0, 1)
	if err.SkipRetry {
...
func formatParamOverrideAuditValue(value interface{}) string {
	...
	case string:
		return typed
	default:
		return platformtext.GetJsonString(typed)
	}
}
...
func checkSingleCondition(data []byte, contextJSON string, condition ConditionOperation) (bool, error) {
	...
	// 利用gjson的类型解析
	targetBytes, err := platformencoding.Marshal(condition.Value)
	if err != nil {
...
func marshalContextJSON(context map[string]interface{}) (string, error) {
	...
	ctxBytes, err := platformencoding.Marshal(context)
	if err != nil {
		return "", err
	}
...
func parseHeaderPassThroughNames(value interface{}) ([]string, error) {
	...
	case string:
		...
		if strings.HasPrefix(trimmed, "[") || strings.HasPrefix(trimmed, "{") {
			...
			if err := platformencoding.UnmarshalString(trimmed, &parsed); err == nil {
...
func expandWildcardPaths(data []byte, path string) ([]string, error) {
	var root interface{}
	if err := platformencoding.Unmarshal(data, &root); err != nil {
		return nil, err
	}
...
func pruneObjects(data []byte, path, contextJSON string, value interface{}) ([]byte, error) {
	...
	if path == "" {
		...
		return platformencoding.Marshal(cleaned)
	}
	...
	if target.Type == gjson.JSON {
		if err := platformencoding.UnmarshalString(target.Raw, &targetNode); err != nil {
			return nil, err
		}
	} else {
	...
	cleanedBytes, err := platformencoding.Marshal(cleaned)
	if err != nil {
		return nil, err
	}
...
func shouldPruneObject(node map[string]interface{}, options pruneObjectsOptions, contextJSON string) (bool, error) {
	nodeBytes, err := platformencoding.Marshal(node)
	if err != nil {
		return false, err
	}
...
func mergeObjects(data []byte, path string, value interface{}, keepOrigin bool) ([]byte, error) {
	...
	if err := platformencoding.UnmarshalString(current.Raw, &currentMap); err != nil {
		return nil, err
	}
	// 解析新值
	switch v := value.(type) {
	case map[string]interface{}:
		newMap = v
	default:
		jsonBytes, _ := platformencoding.Marshal(v)
		if err := platformencoding.Unmarshal(jsonBytes, &newMap); err != nil {
			return nil, err
		}
	}
	// 合并
