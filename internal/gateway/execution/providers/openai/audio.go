...
import (
	...
	"github.com/sh2001sh/CodeGo-Api/internal/platform/textx"
...
func OpenaiTTSHandler(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) *dto.Usage {
	...
	for k, v := range resp.Header {
		if !platformhttpx.ShouldCopyUpstreamHeader(c, k, v) {
			continue
		}
	...
	if info.IsStream {
		helper.StreamScannerHandler(c, resp, info, func(data string, sr *helper.StreamResult) {
			if textx.SundaySearch(data, "usage") {
				var simpleResponse dto.SimpleResponse
				if err := platformencoding.Unmarshal([]byte(data), &simpleResponse); err != nil {
		...
		if audioFormat == "pcm" {
			...
			duration, durationErr = platformfilex.GetAudioDuration(c.Request.Context(), reader, ext)
		}
...
func OpenaiSTTHandler(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo, responseFormat string) (*types.APIError, *dto.Usage) {
	defer platformhttpx.CloseResponseBodyGracefully(resp)
	...
	if err := platformencoding.Unmarshal(responseBody, &responseData); err == nil && responseData.Usage != nil {
