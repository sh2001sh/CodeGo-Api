...
func CovertOpenAI2Gemini(c *gin.Context, textRequest dto.GeneralOpenAIRequest, info *relaycommon.RelayInfo) (*dto.GeminiChatRequest, error) {
	...
	if textRequest.TopP != nil && *textRequest.TopP > 0 {
		geminiRequest.GenerationConfig.TopP = platformruntime.GetPointer(*textRequest.TopP)
	}

	if maxTokens := textRequest.GetMaxTokens(); maxTokens > 0 {
		geminiRequest.GenerationConfig.MaxOutputTokens = platformruntime.GetPointer(maxTokens)
	}

	if textRequest.Seed != nil && *textRequest.Seed != 0 {
		geminiSeed := int64(lo.FromPtr(textRequest.Seed))
		geminiRequest.GenerationConfig.Seed = platformruntime.GetPointer(geminiSeed)
	}

	attachThoughtSignature := (info.ChannelType == constant.ChannelTypeGemini ||
		info.ChannelType == constant.ChannelTypeVertexAi) &&
		gatewaystore.GetGeminiSettings().FunctionCallThoughtSignatureEnabled

	if gatewaystore.IsGeminiModelSupportImagine(info.UpstreamModelName) {
	...
	if len(textRequest.ExtraBody) > 0 {
		...
		if err := platformencoding.Unmarshal(textRequest.ExtraBody, &extraBody); err != nil {
		...
		if googleBody, ok := extraBody["google"].(map[string]interface{}); ok {
			if !strings.HasSuffix(info.UpstreamModelName, "-nothinking") {
				...
				if thinkingConfig, ok := googleBody["thinking_config"].(map[string]interface{}); ok {
					...
					if thinkingBudget, exists := thinkingConfig["thinking_budget"]; exists {
						...
						case float64:
							budgetInt := int(v)
							tempThinkingConfig.ThinkingBudget = platformruntime.GetPointer(budgetInt)
							if budgetInt > 0 {
								// 有正数预算
			...
			if imageConfig, ok := googleBody["image_config"].(map[string]interface{}); ok {
				...
				if len(geminiImageConfig) > 0 {
					imageConfigBytes, err := platformencoding.Marshal(geminiImageConfig)
					if err != nil {
	...
	for _, category := range SafetySettingList {
		safetySettings = append(safetySettings, dto.GeminiChatSafetySettings{
			Category:  category,
			Threshold: gatewaystore.GetGeminiSafetySetting(category),
		})
	}
	...
	if textRequest.ResponseFormat != nil && (textRequest.ResponseFormat.Type == "json_schema" || textRequest.ResponseFormat.Type == "json_object") {
		...
		if len(textRequest.ResponseFormat.JsonSchema) > 0 {
			...
			if err := platformencoding.Unmarshal(textRequest.ResponseFormat.JsonSchema, &jsonSchema); err == nil {
		...
		for _, part := range openaiContent {
			if part.Type == dto.ContentTypeText {
				...
				for {
					...
					format, base64String, err := platformfilex.DecodeBase64FileData(dataUrl)
					if err != nil {
				...
				base64Data, mimeType, err := platformfilex.GetBase64Data(c, source, "formatting image for Gemini")
				if err != nil {
...
func getResponseToolCall(item *dto.GeminiPart) *dto.ToolCallResponse {
	...
	return &dto.ToolCallResponse{
		ID:   fmt.Sprintf("call_%s", platformruntime.GetUUID()),
		Type: "function",
...
func handleStream(c *gin.Context, info *relaycommon.RelayInfo, resp *dto.ChatCompletionsStreamResponse) error {
	streamData, err := platformencoding.Marshal(resp)
	if err != nil {
...
func geminiStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response, callback func(data string, geminiResponse *dto.GeminiChatResponse) bool) (*dto.Usage, *types.APIError) {
	var usage = &dto.Usage{}
	var imageCount int
	...
	helper.StreamScannerHandler(c, resp, info, func(data string, sr *helper.StreamResult) {
		...
		if err := platformencoding.UnmarshalString(data, &geminiResponse); err != nil {
	...
	info.ConversationResponseText = responseText.String()

	return usage, nil
}

func GeminiChatStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.APIError) {
	id := helper.GetResponseID(c)
	createAt := platformruntime.GetTimestamp()
	finishReason := constant.FinishReasonStop
	...
	if handleErr != nil {
		platformobservability.SysLog("send final response failed: " + handleErr.Error())
	}
	return usage, nil
}

func GeminiChatHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.APIError) {
	...
	if len(geminiResponse.Candidates) == 0 {
		...
		if geminiResponse.PromptFeedback != nil && geminiResponse.PromptFeedback.BlockReason != nil {
			...
			httpctx.SetContextKey(c, constant.ContextKeyAdminRejectReason, "gemini_empty_candidates")
			apiError = types.NewOpenAIError(
		...
		platformhttpx.ResetStatusCode(apiError, c.GetString("status_code_mapping"))
	...
	info.ConversationResponseText = geminiOpenAIResponseText(fullTextResponse)

	switch info.RelayFormat {
	case types.RelayFormatOpenAI:
		responseBody, err = platformencoding.Marshal(fullTextResponse)
		if err != nil {
			return nil, types.NewError(err, types.ErrorCodeBadResponseBody)
		}
	case types.RelayFormatClaude:
		claudeResp := gatewaytranslation.ResponseOpenAI2Claude(fullTextResponse)
...
func geminiOpenAIResponseText(response *dto.OpenAITextResponse) string {
	if response == nil {
		return ""
	}
...
func GeminiEmbeddingHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.APIError) {
	...
	if jsonErr := platformencoding.Unmarshal(responseBody, &geminiResponse); jsonErr != nil {
	...
	jsonResponse, jsonErr := platformencoding.Marshal(openAIResponse)
	if jsonErr != nil {
		return nil, types.NewOpenAIError(jsonErr, types.ErrorCodeBadResponseBody, http.StatusInternalServerError)
	}
...
func GeminiImageHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.APIError) {
	responseBody, readErr := io.ReadAll(resp.Body)
	if readErr != nil {
		return nil, types.NewOpenAIError(readErr, types.ErrorCodeBadResponseBody, http.StatusInternalServerError)
	}
