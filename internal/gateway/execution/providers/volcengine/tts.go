...
func handleTTSResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo, encoding string) (usage any, err *types.APIError) {
...
func handleTTSWebSocketResponse(c *gin.Context, requestURL string, volcRequest VolcengineTTSRequest, info *relaycommon.RelayInfo, encoding string) (usage any, err *types.APIError) {
	buildUsage := func() any {
		return &dto.Usage{
			PromptTokens:     info.GetEstimatePromptTokens(),
			CompletionTokens: 0,
	...
	for {
		...
		case MsgTypeAudioOnlyServer:
			if len(msg.Payload) > 0 {
				...
				if flushErr := gatewaystream.FlushWriter(c); flushErr != nil {
					if gatewaystream.IsClientGone(c) {
						return buildUsage(), nil
					}
					return nil, types.NewErrorWithStatusCode(
