...
func handleTTSResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (usage any, err *types.APIError) {
...
func handleChatCompletionResponse(c *gin.Context, resp *http.Response, _ *relaycommon.RelayInfo) (usage any, err *types.APIError) {
	body, readErr := io.ReadAll(resp.Body)
	if readErr != nil {
		return nil, types.NewErrorWithStatusCode(
	...
	for key, values := range resp.Header {
		if !platformhttpx.ShouldCopyUpstreamHeader(c, key, values) {
			continue
		}
