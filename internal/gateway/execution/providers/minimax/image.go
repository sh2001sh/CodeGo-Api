...
func oaiImage2MiniMaxImageRequest(request dto.ImageRequest) MiniMaxImageRequest {
	...
	if raw, ok := request.Extra["prompt_optimizer"]; ok {
		...
		if err := platformencoding.Unmarshal(raw, &promptOptimizer); err == nil {
...
func aspectRatioFromImageRequest(request dto.ImageRequest) string {
	if raw, ok := request.Extra["aspect_ratio"]; ok {
		...
		if err := platformencoding.Unmarshal(raw, &aspectRatio); err == nil && aspectRatio != "" {
			return aspectRatio
		}
	}
...
func responseMiniMax2OpenAIImage(response *MiniMaxImageResponse, info *relaycommon.RelayInfo) (*dto.ImageResponse, error) {
	...
	if len(response.Metadata) > 0 {
		metadata, err := platformencoding.Marshal(response.Metadata)
		if err != nil {
			return nil, err
		}
...
func miniMaxImageHandler(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (*dto.Usage, *types.APIError) {
	...
	if err := platformencoding.Unmarshal(responseBody, &minimaxResponse); err != nil {
	...
	jsonResponse, err := platformencoding.Marshal(openAIResponse)
	if err != nil {
		return nil, types.NewError(err, types.ErrorCodeBadResponseBody)
	}
