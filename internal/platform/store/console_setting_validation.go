...
func parseConsoleJSONArray(jsonStr string, typeName string) ([]map[string]interface{}, error) {
...
func validateConsoleURL(urlStr string, index int, itemType string) error {
...
func checkDangerousConsoleContent(content string, index int, itemType string) error {
...
func getConsoleJSONList(jsonStr string) []map[string]interface{} {
	if jsonStr == "" {
...
func ValidateConsoleSettings(settingsStr string, settingType string) error {
	...
	switch settingType {
	case "ApiInfo":
		return validateConsoleAPIInfo(settingsStr)
	case "Announcements":
		return validateConsoleAnnouncements(settingsStr)
	case "FAQ":
		return validateConsoleFAQ(settingsStr)
	case "UptimeKumaGroups":
		return validateConsoleUptimeKumaGroups(settingsStr)
	default:
		return fmt.Errorf("未知的设置类型：%s", settingType)
	}
}

func validateConsoleAPIInfo(apiInfoStr string) error {
	apiInfoList, err := parseConsoleJSONArray(apiInfoStr, "API信息")
	if err != nil {
		return err
	}
	...
	for i, apiInfo := range apiInfoList {
		...
		if err := validateConsoleURL(urlStr, i+1, "API信息"); err != nil {
			return err
		}
		...
		if err := checkDangerousConsoleContent(description, i+1, "API信息"); err != nil {
			return err
		}
		if err := checkDangerousConsoleContent(route, i+1, "API信息"); err != nil {
			return err
		}
	}
	return nil
...
func GetAPIInfo() []map[string]interface{} {
	return getConsoleJSONList(GetConsoleSetting().ApiInfo)
}

func validateConsoleAnnouncements(announcementsStr string) error {
	list, err := parseConsoleJSONArray(announcementsStr, "系统公告")
	if err != nil {
		return err
	}
	if len(list) > 100 {
	...
	for i, ann := range list {
		...
		if t, exists := ann["type"]; exists {
			if typeStr, ok := t.(string); ok && !validTypes[typeStr] {
...
func validateConsoleFAQ(faqStr string) error {
	list, err := parseConsoleJSONArray(faqStr, "FAQ信息")
	if err != nil {
		return err
	}
	if len(list) > 100 {
...
func getConsolePublishTime(item map[string]interface{}) time.Time {
...
func GetAnnouncements() []map[string]interface{} {
	list := getConsoleJSONList(GetConsoleSetting().Announcements)
	sort.SliceStable(list, func(i, j int) bool {
		return getConsolePublishTime(list[i]).After(getConsolePublishTime(list[j]))
	})
	return list
}

func GetFAQ() []map[string]interface{} {
	return getConsoleJSONList(GetConsoleSetting().FAQ)
}

func validateConsoleUptimeKumaGroups(groupsStr string) error {
	groups, err := parseConsoleJSONArray(groupsStr, "Uptime Kuma分组配置")
	if err != nil {
		return err
	}
	...
	for i, group := range groups {
		...
		if err := validateConsoleURL(urlStr, i+1, "分组"); err != nil {
			return err
		}
		...
		if err := checkDangerousConsoleContent(description, i+1, "分组"); err != nil {
			return err
		}
		if err := checkDangerousConsoleContent(categoryName, i+1, "分组"); err != nil {
			return err
		}
	}
	return nil
...
func GetUptimeKumaGroups() []map[string]interface{} {
	return getConsoleJSONList(GetConsoleSetting().UptimeKumaGroups)
}
